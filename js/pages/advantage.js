...
export function renderAdvantage(main) {
  ...
  const tiers = feeDrag(S.corpus, S.monthly, gross, yrs, [
    { key: 'liqd', label: 'LIQD flat fee', short: 'LIQD', color: '#4dd6c1', flatFee: tierFee(), pctFee: 0 },
    { key: 'direct', label: 'DIY direct funds', short: 'DIY direct', color: '#3987e5', pctFee: 0.15 },
    { key: 'pms', label: 'PMS 2% + 20% perf', short: 'PMS', color: '#c98500', pctFee: 2, perfShare: 20, hurdle: 10 },
    { key: 'regular', label: 'Regular MF (distributor)', short: 'Regular MF', color: '#e66767', pctFee: 1.4 },
  ]);
  ...
  main.innerHTML = `
    <div class="page-head">
      ...
      <p class="page-sub">Same gross return (${fmtPct(gross, 1)}), same ${yrs} years, same ${fmtINR(S.corpus)} + ${fmtINR(S.monthly)}/mo. The only variable is who takes a cut, and how.</p>
    </div>
    ...
    <div class="card pad-lg" style="margin-top:18px">
      ...
      <div class="grid g3">
        ...
        ].map(([t, p, d]) => `<div class="card" style="background:var(--surface-2)">
  ...
  main.querySelector('#feeTiles').innerHTML = tiers.map(t => `
    <div class="card stat" style="${t.key === 'liqd' ? 'border-color:rgba(77,214,193,0.4)' : ''}">
  ...
  main.querySelector('#pillars').innerHTML = PILLARS.map(([g, t, d]) => `
    <div class="card"><div style="font-size:22px">${g}</div>
      <b style="display:block;margin:10px 0 6px;font-size:15px">${t}</b>
      <p class="small dim">${d}</p></div>`).join('');
}
